...
template <typename T>
class ElasticPartitionOpGPU : public AsyncOpKernel {
 public:
  explicit ElasticPartitionOpGPU(OpKernelConstruction* c) : AsyncOpKernel(c) {
  ...
  void AllocateOutputs(OpKernelContext* c, const Tensor* data,
                       ...
                       OpOutputList* data_out, OpOutputList* ind_out,
                       DoneCallback done) {
    auto e_part_count = partition_count->flat<int32>();
    // Allocate output tensors of the right size
    OP_REQUIRES_OK_ASYNC(c, c->output_list("p_data", data_out), done);
    OP_REQUIRES_OK_ASYNC(c, c->output_list("p_indices", ind_out), done);
    for (int p = 0; p < num_partitions_; p++) {
      ...
      Tensor* out;
      OP_REQUIRES_OK_ASYNC(c, data_out->allocate(p, shape, &out), done);
      OP_REQUIRES_OK_ASYNC(
          c, data_out->allocate(p + num_partitions_, shape, &out), done);
    }
  }
  ...
  void ComputeAsync(OpKernelContext* c, DoneCallback done) {
    ...
    if (partitions.NumElements() == 0) {
      ...
      OpOutputList data_outputs;
      OpOutputList indices_outputs;
      this->AllocateOutputs(c, &data, &partitions, &partition_count,
                            &data_outputs, &indices_outputs, done);
      if (c->status().ok()) done();
      return;
    }
...
#define REGISTER_DYNAMIC_PARTITION_GPU(T)                                 \
  REGISTER_KERNEL_BUILDER(                                                \
      Name("ElasticPartition").Device(DEVICE_GPU).TypeConstraint<T>("T"), \
      ElasticPartitionOpGPU<T>)
